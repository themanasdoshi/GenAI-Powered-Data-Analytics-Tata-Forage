...
def run_delinquency_model():
    ...
    # NEW FEATURE 1: Aggregated Payment History Score
    # Weighted sum, where recent months (Month_6) have a higher weight
    df['Payment_History_Score'] = (
        ...
        df['Month_5_Num'] * 5 +
        df['Month_6_Num'] * 6
    ) / 21.0 # Divide by the sum of weights (1+2+3+4+5+6) to normalize

    # NEW FEATURE 2: Credit Utilization / Credit Score Ratio
    # Captures the severity of debt load relative to creditworthiness
    df['Util_Score_Ratio'] = df['Credit_Utilization'] / (df['Credit_Score'] + 1e-6) 

    # NEW FEATURE 3: Income to Loan Balance Ratio
    # Captures ability to service the loan relative to income
    ...
    # Define the complete list of features (X), excluding original month columns
    features = [
        'Age', 'Income', 'Credit_Score', 'Credit_Utilization', 'Missed_Payments', 
        'Loan_Balance', 'Debt_to_Income_Ratio', 'Account_Tenure',
        'Payment_History_Score', 'Util_Score_Ratio', 'Income_Loan_Ratio' # New engineered features
    ] + [
    ...
    # Random Forest Classifier with 'balanced' class weight and refined hyperparameters
    rf_model = RandomForestClassifier(
        n_estimators=100, 
        random_state=42, 
        class_weight='balanced',
        max_depth=10, 
        min_samples_leaf=5 # Increased regularization to reduce reliance on noisy individual data points
    )
    ...
    print("\n" + "="*40)
    print("      DELINQUENCY MODEL PERFORMANCE (V2)")
    print("="*40)
